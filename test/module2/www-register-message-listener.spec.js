describe('bin/www', () => {
  it('should contain ws `message` listener @www-register-message-listener', () => {
    ...
    assert(
      matchObj(message, messageArrowFunction) ||
        matchObj(message, messageHandlerFunction),
      'Do you have a `message` event handler function with a `message` parameter?',
    );
  });
});
